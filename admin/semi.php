...
<head>
    ...
    <title>Semi Interested | Admin Panel</title>
...
<body>
	...
	<div class="container--table">
		...
		<div class="d-md-flex justify-content-between">
			...
			<form action="export.php" method="post">
				<input type="hidden" name="table" value="semi_drivers">
					...
					<td class="text-center align-middle">
	   				...
	   				<td class="text-center align-middle">
						<button title="Change Status" class="btn btn-success btn-sm" data-id="<?php echo $row['id']; ?>" data-toggle="modal" data-target="#statusModal"><i class="fa fa-clock-o"></i></button>
	   					...
	   					<a title="Edit Info" class="btn btn-primary btn-sm mx-1" href="semiedit.php?id=<?php echo $row['id']; ?>"><i class="fa  fa-pencil"></i></a>
  ...
  <script>
	...
	$('#detailsModal').on('show.bs.modal', function (event) {
		...
		modal.find('.modal-body').load(`view/semiview.php?id=${id}`);
	...
	$('#statusModal').on('show.bs.modal', function (event) {
		...
		modal.find('.modal-body').load(`status.php?table=semi_drivers&id=${id}`);
	});
	...
	function statusUpdate (updateButton) {
		...
		$.ajax({
			url: 'process.php',
			method: 'post',
			data: {key: 'statusUpdate', id: id, status: status, table: 'semi_drivers'},
			success: function(response) {
				if (response.includes('statusSuccess')) {
					window.location.href = 'semi.php';
				} else {
	...
	$('.deleteBtn').click(function(e) {
		...
		if (deleteItem) {
			$.ajax({
				url: 'process.php',
				method: 'POST',
				data: {key: 'delete', id: id, table: 'semi_drivers'},
				success: function(response) {
		...
		if (checkedItems.length > 0) {
			...
			if (deleteSelectedItems) {
				$.ajax({
					url: 'process.php',
					method: 'POST',
					data: {key: 'deleteSelected', ids: checkedItems, table: 'semi_drivers'},
					success: function(response) {
						if (response.includes('deleteSuccess')) {
							window.location.href = `semi.php`;
						} else {
