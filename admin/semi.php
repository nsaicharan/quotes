...
<body>
	...
	<div class="container--table">
		...
		<h2 class="text-center">Semi Interested Listing</h2>
			...
			<tbody class="text-center">
	   			...
	   			<tr>
					<td class="text-center align-middle">
	   				...
	   				<td class="text-center align-middle">
						<a title="Edit" class="btn btn-primary btn-sm mr-1" href="semiedit.php?id=<?php echo $row['id']; ?>"><i class="fa  fa-pencil"></i></a><button title="Delete" class="deleteBtn btn btn-danger btn-sm ml-1" data-id="<?php echo $row['id']; ?>"><i class="fa fa-trash-o"></i></button>
					</td>
	   			</tr>
	...
	$('.deleteBtn').click(function(e) {
		...
		if (deleteItem) {
			$.ajax({
				url: 'process.php',
				method: 'POST',
				data: {key: 'delete', id: id, table: 'semi'},
		...
		if (checkedItems.length > 0) {
			...
			if (deleteSelectedItems) {
				$.ajax({
					url: 'process.php',
					method: 'POST',
					data: {key: 'deleteSelected', ids: checkedItems, table: 'semi'},
					success: function(response) {
						if (response.includes('deleteSuccess')) {
							window.location.href = `semi.php`;
						} else {
