	...
	if (!$_SESSION['loggedInUser']) {
		...
		if ( isset($_POST['key']) ) {
			...
			if ( $_POST['key'] == 'statusUpdate') {
	    		...
	    		if($result) {
					...
					if ($status == 'Sell Back') {
						...
						$formquery = "SELECT btn_one_text, btn_two_text, btn_three_text FROM bg_logo";
						$formresult = mysqli_query($conn, $formquery);
						$formrow = mysqli_fetch_array($formresult);

						if ($table == 'quick') {
							$form = $formrow['btn_one_text'];
						} else if ($table == 'semi_drivers') {
							$form = $formrow['btn_two_text'];
						} else {
							$form = $formrow['btn_three_text'];
						}
			...
			if ( $_POST['key'] == 'delete' ) {
				...
				if ($table == 'semi_drivers') {
					$query = "DELETE FROM semi_vehicles WHERE driver = '$id'";
					$result = mysqli_query($conn, $query);
				}
				
				if ($table == 'ready_drivers') {
					$query = "DELETE FROM ready_vehicles WHERE driver = '$id'";
					$result = mysqli_query($conn, $query);
				}
			...
			if ( $_POST['key'] == 'siteInfo' ) {
				...
				$frontPageText = mysqli_real_escape_string($conn, $_POST['frontPageText']);

				$query = "UPDATE bg_logo SET site_title = '$siteTitle', front_page_text = '$frontPageText'";
				$result = mysqli_query($conn, $query);
			...
			//General Styles
			if ( $_POST['key'] == 'generalStyles' ) {

				$primaryColor = mysqli_real_escape_string($conn, $_POST['primaryColor']);
				$layerOpacity = mysqli_real_escape_string($conn, $_POST['layerOpacity']);

				$query = "UPDATE form_styles SET primary_color = '$primaryColor', layer_opacity = '$layerOpacity'";
				$result = mysqli_query($conn, $query);
			...
			if ( $_POST['key'] == 'formText' ) {
				...
				if ( isset($_POST['formOne']) ) {
					$query = "UPDATE form_styles SET form_one_title = '$formOneTitle', form_one_subtitle = '$formOneSubtitle'";
				} else if ( isset($_POST['formTwo']) ) {
					$query = "UPDATE form_styles SET form_two_driver = '$formTwoDriver', form_two_vehicle = '$formTwoVehicle'";
				} else {
					$query = "UPDATE form_styles SET form_three_driver = '$formThreeDriver', form_three_vehicle = '$formThreeVehicle'";
				}

				$result = mysqli_query($conn, $query);

				if ($result) {
					echo "formTextSuccess";
				} else {
					echo mysqli_error($conn);
				}
			}
